...
use Risk\Common\Models\UserData\VarParametersSystemAppReloan;

class RiskReloanUserAppServer extends RiskUserAppServer {
    ...
    public function __construct() {
        $this->systemapps = VarParametersSystemAppReloan::model()->where('APP_type', self::SYSTEM_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->loanapps_app_name = VarParametersSystemAppReloan::model()->where('APP_type', self::LOAN_APP_TYPE)
            ->where('remark', self::REMARK_APP_NAME)->pluck('pkg_name')->toArray();
        $this->loanapps_pkg_name_not = VarParametersSystemAppReloan::model()->where('APP_type', self::LOAN_APP_TYPE)
            ->where('remark', self::REMARK_PKG_NAME_NOT_IN)->pluck('pkg_name')->toArray();
        $this->gloanapps_app_name = VarParametersSystemAppReloan::model()->where('APP_type', self::GLOAN_APP_TYPE)
            ->where('remark', self::REMARK_APP_NAME)->pluck('pkg_name')->toArray();
        $this->gloanapps_pkg_name_small = VarParametersSystemAppReloan::model()->where('APP_type', self::GLOAN_APP_TYPE)
            ->where('remark', self::REMARK_PKG_NAME_SMALL)->pluck('pkg_name')->toArray();
        $this->prmapps = VarParametersSystemAppReloan::model()->where('APP_type', self::PRM_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->upapps = VarParametersSystemAppReloan::model()->where('APP_type', self::UP_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->sbapps = VarParametersSystemAppReloan::model()->where('APP_type', self::SB_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->shopapps = VarParametersSystemAppReloan::model()->where('APP_type', self::SHOP_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->btmapps = VarParametersSystemAppReloan::model()->where('APP_type', self::BTM_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->bankapps_app_name = VarParametersSystemAppReloan::model()->where('APP_type', self::BANK_APP_TYPE)
            ->where('remark', self::REMARK_APP_NAME)
            ->pluck('pkg_name')->toArray();
        $this->bankapps_pkg_name_small = VarParametersSystemAppReloan::model()->where('APP_type', self::BANK_APP_TYPE)
            ->where('remark', self::REMARK_PKG_NAME_SMALL)
            ->pluck('pkg_name')->toArray();
        $this->payapps = VarParametersSystemAppReloan::model()->where('APP_type', self::PAY_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->goodapps = VarParametersSystemAppReloan::model()->where('APP_type', self::GOOD_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->badapps = VarParametersSystemAppReloan::model()->where('APP_type', self::BAD_APP_TYPE)->pluck('pkg_name')->toArray();
        $this->newgoodapps = VarParametersSystemAppReloan::model()->where('APP_type', self::NEW_GOOD_APP_TYPE)->pluck('pkg_name')->toArray();
    }

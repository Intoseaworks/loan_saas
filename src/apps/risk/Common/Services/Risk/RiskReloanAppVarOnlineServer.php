...
class RiskReloanAppVarOnlineServer {
    ...
    public function getBadApp() {
        $computeUserapplicationDifCreateInstall = new ComputeUserapplicationDifCreateInstall($this->_order);
        $computeUserapplicationDifCreateInstall->handle();
    }
    ...
    public function getMaxIntLoanist() {
        $key = "Order::min_int_appist::OrderID::{$this->_order->id}".date("Ymd");
        $min_int_appist = Cache::remember($key, 10, function(){
            return UserApplicationDiffCreateInstall::model()
            ->select(\DB::raw('min(dif_create_install) as mindif,max(dif_create_install) as maxdif'))
            ->whereOrderId($this->_order->id)
            ->whereIsAdjSystem(0)->first();
        });
        return round(optional($min_int_appist)->maxdif,2);
    }
    ...
    public function getMinIntAppist() {
        ...
        return round(optional($min_int_appist)->mindif,2);
    }
    ...
    public function getNsysLoanInM1() {
        ...
        ->whereIsAdjSystem(0)
        ->whereIsLoan(1)
        ->where('dif_create_install','<=',30)->count();
    }

    public function getStdCommonInstallDif() {
        $key = "Order::std_create_install_dif::OrderID::{$this->_order->id}".date("Ymd");
        ...
        return $std_create_install_dif->std_create_install_dif;
    }

    public function getLastOrderOd() {
        $firstFinishOrder = Order::model()->whereUserId($this->_order->user_id)->whereIn('status',Order::FINISH_STATUS)->orderByDesc('id')->first();
        if ( optional($firstFinishOrder)->status == Order::STATUS_OVERDUE_FINISH ){
            return true;
        }else{
            return false;
        }

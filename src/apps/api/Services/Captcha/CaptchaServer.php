...
use Common\Utils\Sms\SmsCaptchaHelper;
...
class CaptchaServer extends BaseService
{
     ...
     * @throws ApiException
     */
    public function sendSmsCode($phone, $use = SmsCaptchaRedis::USE_APP_LOGIN)
    {
        if (!UserBlack::model()->canLogin($phone)) {
            throw new ApiException('The phone cannot be logged in');
        }
        $code = SmsCaptchaHelper::getCaptchaRandCode();
        $helper = SmsCaptchaHelper::helper();
        if (!$helper->send($phone, $code, $use)) {
            throw new ApiException($helper->getError());
        }
        return true;
    }

    public function validateSmsCode($captcha, $telephone, $use = SmsCaptchaRedis::USE_APP_LOGIN)
    {
        return SmsCaptchaHelper::helper()->validate($captcha, $telephone, null, $use);
    }
     ...
     */
    public function sendSms($telephone, $use)
    {
        $code = SmsCaptchaHelper::getCaptchaRandCode();
        $helper = SmsCaptchaHelper::helper();
        if (!$helper->send($telephone, $code, $use)) {
            throw new ApiException($helper->getError());
        }
    ...
    public function sendVoice($telephone)
    {
        $code = SmsCaptchaHelper::getCaptchaRandCode();
        $helper = SmsCaptchaHelper::helper();
        $use = SmsCaptchaRedis::USE_APP_LOGIN;
        if (!$helper->send($telephone, $code, $use, null, [], 'voice')) {
            throw new ApiException($helper->getError());
        }
